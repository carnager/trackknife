...
| Embedded storage and fallback | Partial | Conventional ReplayGain values use the ordinary qualified text-write pipeline; CUE tracks use the sheet (ADR-0139), other logical tracks the sidecar (ADR-0141). An explicit sidecar fallback for whole-file tracks in unwritable formats and a qualified Opus R128 policy are missing. Scanning must remain independent of tag writability. |
