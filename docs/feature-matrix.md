...
| MPD browse and search | Implemented | Artist/album browsing, numbered tracks and covers, A–Z/Latest ordering, Go to artist/album, bounded library-integrated search and continuation, and append/next/replace actions. Search survives focus and authority changes (ADR-0120). A user-facing custom tree editor remains open. |
...
| Optional local library | Implemented | Chosen roots, incremental revision checks, paged artist/album/track browsing and search, offline retention, numbered tracks, covers, context actions, and local-only drops. **Only Refresh starts a filesystem scan.** Tag/move commits update the cached index transactionally without rescanning (ADRs 0115–0118). |
